...
describe("JService", function () {
  it("should allow category requests", function (done) {
    ...
    jserviceApi.get("/category?id=10044").reply(200, {
      title: expectedCategory,
    });

    jservice.getCategory(10044, function (category) {
      assert.equal(category.category, expectedCategory);
      assert.equal(category.categoryApiCall, undefined);
      done();
    });
  });
  ...
  it("should get random questions", function (done) {
    ...
    jserviceApi.get("/random").reply(200, [
      {
  ...
  it("should allow make category call", function (done) {
    var expectedCategory = "my category";
    ...
    jservice.getCategoryCall(10044, function (category) {
      assert.equal(category.categoryApiCall, true);
      done();
    });
  });
});
